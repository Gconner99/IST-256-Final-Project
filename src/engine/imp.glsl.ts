...
Fig impRoll(float seed, float time) {
  ...
  f.ts = vec3(
    mix(0.28, 0.52, figH(seed + 1.2)),
    mix(0.07, 0.16, pow(figH(seed + 1.3), 0.9)),
    mix(0.2, 0.4, figH(seed + 1.4))
  );
  f.sx = mix(1.05, 1.45, figH(seed + 1.22));
  f.sz = mix(0.95, 1.32, figH(seed + 1.26));
  f.hs = mix(0.48, 0.82, pow(figH(seed + 2.3), 0.5));
  f.neck = mix(0.0, 0.08, figH(seed + 2.05));
  f.horn = 1.0;
  f.tail = 1.0;
  f.ears = 1.0;
  ...
  f.pack = 0.0;
  f.orb = 0.0;
  f.blush = step(0.45, figH(seed + 0.74));
  f.extraLeg = 0.0;
  f.arms = 2.0;
  f.nEyes = 1.0;
  f.eyeY = f.hs * mix(0.08, 0.18, figH(seed + 6.2));
  f.eyeZ = -f.hs * mix(0.7, 0.95, figH(seed + 6.3));
  f.eyeSpread = f.hs * mix(0.55, 0.85, figH(seed + 6.4));
  f.eyeR = f.hs * mix(0.16, 0.28, figH(seed + 6.5));
  f.eyeSq = mix(0.28, 0.5, figH(seed + 6.55));
  f.lean += 0.28;
  f.kickAmt *= 1.2;
  return f;
}
vec2 impFace(vec3 hp, Fig f) {
  float hs = f.hs;
  vec2 d = vec2(figBox(hp, vec3(hs * 1.08, hs * 0.88, hs * 0.55)), 2.0);
  d = figMin(d, vec2(figOcta(hp * vec3(1.15, 0.72, 1.0), hs * 1.22), 2.0));
  d = figMin(d, vec2(figBox(hp - vec3(0.0, hs * 0.06, -hs * 0.72), vec3(hs * 1.02, hs * 0.55, hs * 0.16)), 2.4));
  vec3 slit = hp - vec3(0.0, f.eyeY, f.eyeZ);
  slit.y *= f.eyeSq;
  d = figMin(d, vec2(figBox(slit, vec3(f.eyeSpread, f.eyeR * 0.38, f.eyeR * 0.5)), 5.0));
  d = figMin(d, vec2(figBox(slit - vec3(0.0, 0.0, -f.eyeR * 0.25), vec3(f.eyeSpread * 0.72, f.eyeR * 0.22, f.eyeR * 0.28)), 5.6));
  d = figMin(d, vec2(figBox(hp - vec3(0.0, -hs * 0.28, -hs * 0.95), vec3(hs * 0.38, hs * 0.1, hs * 0.22)), 7.0));
  d = figMin(d, vec2(figCap(hp, vec3(0.0, -hs * 0.18, -hs * 0.4), vec3(0.0, -hs * 0.38, -hs * 1.45), hs * 0.12), 6.0));
  d = figMin(d, vec2(figCap(hp, vec3(-hs * 0.62, hs * 0.22, 0.12), vec3(-hs * 1.45, hs * 0.95, 0.22), hs * 0.09), 7.5));
  d = figMin(d, vec2(figCap(hp, vec3(hs * 0.62, hs * 0.2, 0.12), vec3(hs * 1.38, hs * 0.88, 0.18), hs * 0.085), 7.5));
  if (f.tusks > 0.5) {
    d = figMin(d, vec2(figCap(hp, vec3(-hs * 0.26, -hs * 0.32, -hs * 0.7), vec3(-hs * 0.38, -hs * 0.82, -hs * 1.2), hs * 0.055), 8.0));
    d = figMin(d, vec2(figCap(hp, vec3(hs * 0.26, -hs * 0.32, -hs * 0.7), vec3(hs * 0.38, -hs * 0.82, -hs * 1.2), hs * 0.055), 8.0));
  }
  if (f.blush > 0.5) {
    d = figMin(d, vec2(length(hp - vec3(-hs * 0.58, -hs * 0.08, f.eyeZ * 0.5)) - hs * 0.13, 6.9));
    d = figMin(d, vec2(length(hp - vec3(hs * 0.58, -hs * 0.08, f.eyeZ * 0.5)) - hs * 0.13, 6.9));
  }
  return d;
}
vec2 impHit(vec3 p, Fig f, float seed) {
  ...
  p.x *= f.sx;
  p.z *= f.sz;
  vec3 belly = p - vec3(0.0, -f.ts.y * 0.35, 0.06);
  vec2 d = vec2(figBox(belly, vec3(f.ts.x * 1.28, f.ts.y * 0.85, f.ts.z * 1.12)), 1.0);
  d = figMin(d, vec2(figOcta(belly * vec3(0.7, 1.35, 0.85), mix(0.26, 0.4, figH(seed + 1.5))), 1.0));
  vec3 hp = p - vec3(0.0, f.ts.y + mix(0.28, 0.4, figH(seed + 2.1)) + f.neck, 0.02);
  hp = figRotZ(hp, sin(f.t * 4.1) * 0.06);
  hp = figRotX(hp, cos(f.t * 3.2) * 0.04 - f.peck * 0.45);
  d = figMin(d, vec2(figBox(hp, vec3(f.hs * 1.12, f.hs * 0.95, f.hs * 0.72)), 2.0));
  if (length(hp) < f.hs * 3.1) d = figMin(d, impFace(hp, f));
  float hCurl = 0.55 + 0.18 * sin(f.t * 2.0);
  d = figMin(d, vec2(figCap(hp, vec3(-f.hs * 0.42, f.hs * 0.42, 0.08), vec3(-f.hs * (1.15 + hCurl), f.hs * 1.35, 0.32), 0.06), 4.0));
  d = figMin(d, vec2(figCap(hp, vec3(f.hs * 0.42, f.hs * 0.4, 0.08), vec3(f.hs * (1.08 + hCurl), f.hs * 1.28, 0.28), 0.055), 4.0));
  float legLen = mix(0.14, 0.24, figH(seed + 3.3));
  float legR = mix(0.06, 0.11, figH(seed + 3.4));
  for (int i = 0; i < 2; i++) {
    float side = float(i) < 0.5 ? -1.0 : 1.0;
    float kick = sin(f.t * f.kickHz + float(i) * 3.14159) * f.kickAmt;
    vec3 lp = p - vec3(side * f.ts.x * 0.82, -f.ts.y * 0.95, 0.08);
    lp = figRotX(lp, 0.85 + kick);
    lp = figRotZ(lp, side * 0.32);
    d = figMin(d, vec2(figCap(lp, vec3(0.0), vec3(0.0, -legLen, 0.1), legR), 3.0));
    d = figMin(d, vec2(figBox(lp - vec3(0.0, -legLen, 0.12), vec3(0.11, 0.03, 0.16)), 3.0));
  }
  float armR = mix(0.045, 0.08, figH(seed + 4.5));
  for (int i = 0; i < 2; i++) {
    float side = float(i) < 0.5 ? -1.0 : 1.0;
    float wave = sin(f.t * mix(4.2, 7.4, figH(seed + 4.1)) + float(i) * 1.9);
    vec3 ap = p - vec3(side * f.ts.x * 1.15, f.ts.y * 0.02, 0.04);
    ap = figRotZ(ap, side * (1.05 + wave * 0.45));
    vec3 tip = vec3(side * 0.22, -0.12, 0.16);
    d = figMin(d, vec2(figCap(ap, vec3(0.0), tip, armR), 4.0));
    d = figMin(d, vec2(figBox(ap - tip, vec3(0.09, 0.035, 0.12)), 4.0));
  }
  vec3 tb = vec3(0.0, -f.ts.y * 0.2, f.ts.z * 0.7);
  vec3 tm = tb + vec3(sin(f.t * 3.4) * 0.28, 0.18, -0.42);
  vec3 te = tm + vec3(sin(f.t * 3.4 + 0.8) * 0.22, -0.12, -0.38);
  d = figMin(d, vec2(figCap(p, tb, tm, 0.06), 1.5));
  d = figMin(d, vec2(figCap(p, tm, te, 0.048), 1.5));
  d = figMin(d, vec2(figOcta(p - te, 0.11), 6.9));
